...
public class Kiosk {
    public void start() {
        ...
        burgerItems.add(new Burger("더블 치즈버거", 5.9, "두 장의 치즈와 버터가 녹아내리는 부드러운 패티."));
        burgerItems.add(new Burger("불고기 버거", 8.9, "달콤한 불고기 소스와 부드러운 패티, 양상추가 어우러진 맛."));
        burgerItems.add(new Burger("매운 치킨 버거", 5.4, "매콤하고 바삭한 치킨 패티와 양념이 어우러진 매운 맛."));
        burgerItems.add(new Burger("베이컨 치즈버거", 7.5, "바삭한 베이컨과 치즈가 풍미를 더한 클래식한 햄버거."));

        List<MenuItem> drinksItems = new ArrayList<>();
        drinksItems.add(new Drinks("콜라", 2.9, "상쾌하고 달콤한 탄산 음료."));
        drinksItems.add(new Drinks("레모네이드", 8.9, "상큼하고 시원한 레몬의 맛"));
        drinksItems.add(new Drinks("바닐라 밀크셰이크", 5.9, "달콤하고 부드러운 맛"));
        drinksItems.add(new Drinks("아이스 카페라떼", 5.4, "커피와 차가운 우유가 어우러져, 커피 애호가들에게 추천하는 음료."));

        List<MenuItem> dessertsItems = new ArrayList<>();
        dessertsItems.add(new Dessert("초코 브라우니", 7.9, "진한 초콜릿의 풍미가 가득한 디저트"));
        dessertsItems.add(new Dessert("치즈케이크", 8.9, "부드럽고 고소한 치즈의 맛이 진하게 느껴지는 디저트"));
        dessertsItems.add(new Dessert("소프트 아이스크림", 5.9, "부드럽고 달콤한 아이스크림"));
        dessertsItems.add(new Dessert("애플 파이", 5.4, "바삭한 페이스트리와 달콤한 사과가 조화를 이루는 클래식한 디저트."));
